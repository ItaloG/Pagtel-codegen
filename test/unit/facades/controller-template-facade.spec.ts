...
describe("#Controller Facade", () => {
  ...
  it("should return a info if the template file already exits", async () => {
    ...
    const expected = {
      type: "info",
      message: "Controller file already exists",
    };
    const result = await generateControllerFacade({
      name: "any_name",
      scope: "any_scope",
    });
  ...
  it("should create a folder if it not exists", async () => {
    ...
    const expected = {
      filePath: "test/integration/temp/src/presentation/controllers/index.ts",
  ...
  it("should create a controller template file", async () => {
    jest.spyOn(File, "verifyExists").mockReturnValueOnce(false);
    jest.spyOn(Folder, "verifyExists").mockReturnValueOnce(true);

    const fileCreateSpy = jest.spyOn(File, "create");

    const expected = { type: "success", message: "controller generated" };
    const result = await generateControllerFacade({
      name: "any_name",
      scope: "any_scope",
    });
